...
class PrioritizedReplayMemory(object):
    ...
    def push(self, state, action, next_state, reward, td_error):
        ...
        if len(self.memory) < self.capacity:
            self.memory.append(None)
        self.memory[self.position] = Transition(state, action, next_state, reward)
        self.td_error_memory[self.position] = td_error
        self.position = (self.position + 1) % self.capacity

    def sample(self, batch_size):
        # TD誤差の和を計算
        sum_absolute_td_error = np.sum(np.absolute(self.td_error_memory))
        sum_absolute_td_error += TD_ERROR_EPSILON * len(self.td_error_memory)  # 微小値を足す
        ...
        for rand_num in rand_list:
            while tmp_sum_absolute_td_error < rand_num:
                tmp_sum_absolute_td_error += (
                    abs(self.td_error_memory[idx]) + TD_ERROR_EPSILON)
                idx += 1
            ...
            indexes.append(idx)
        return [self.memory[n] for n in indexes]
...
class DQN(nn.Module):
    ...
    def forward(self, x):
        ...
        # torch.Size(B,C,H,W)
        x = self.feature(x)
        x = x.view(x.size()[0], -1) # 画像形式を1次元に変換
        x = F.relu(self.fc(x))
...
class DuelingNet(nn.Module):
    ...
    def __init__(self, output_size: int):
        ...
        self.feature = nn.Sequential(
                ...
                nn.Conv2d(64, 64, kernel_size=3, stride=1),
                nn.ReLU(inplace=True))
        self.adv1 = nn.Linear(64 * 7 * 7, 512)
        self.val1 = nn.Linear(64 * 7 * 7, 512)
        self.adv2 = nn.Linear(512, output_size)
    ...
    def forward(self, x):
        ...
        x = self.feature(x)
        x = x.view(x.size()[0], -1) # 画像形式を1次元に変換
        adv1 = F.relu(self.adv1(x))
        val1 = F.relu(self.val1(x))
        adv2 = self.adv2(adv1) # ここではreluしない
        val2 = self.val2(val1) # ここではreluしない
        output = val2 + adv2 - adv2.mean(1, keepdim=True).expand(-1, adv2.size(1))
        return output
...
class Brain:
    def __init__(self, param, num_actions):
        ...
        if param.prioritized: # 優先度付き経験再生
            self.memory = PrioritizedReplayMemory(self.CAPACITY)
...
def wrap_breakout_env(env, seed, episode_life=True, clip_rewards=False, frame_stack=False, scale=False):
...
if __name__ == "__main__":
    ...
    env = wrap_breakout_env(env, seed=config["env_param"]["seed_num"], frame_stack=config["env_param"]["frame_stack"], scale=config["env_param"]["scale"])  
    ...
    brain_param = BrainParameter(
        ...
        prioritized=config["brain_param"]["prioritized"],
        dueling=config["brain_param"]["dueling"],
